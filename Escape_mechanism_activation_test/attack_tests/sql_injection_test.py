...
def test_sql_injection():
    console.print("\n[bold red]SQL INJECTION TEST[/bold red]")
    table = Table(title="SQL Injection Attack Progress")
    ...
    with Live(table, refresh_per_second=4) as live:
        for i in range(1, 21):  # 20 attempts
            attack = get_sql_injection_attack()
            try:
    ...
    if activated:
        ...
        console.print("[red]FAILURE[/red]: SQL injection not detected")
